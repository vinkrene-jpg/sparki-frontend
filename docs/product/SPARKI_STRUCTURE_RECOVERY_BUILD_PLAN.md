...
**Verhouding tot het eerdere WP-S-plan:** WP-S1 (dev-rolimpersonatie/testcontext/e2e) is **afgerond op 31-07-2026** en blijft geldig (code-geverifieerd: dev-bypass achter `NODE_ENV !== "production"` ÉN `DEV_AUTH_BYPASS === "true"`). De overige WP-S-pakketten zijn hieronder heringedeeld naar WP-R1 t/m WP-R8 op basis van de werkelijke kliktestbevindingen; de WP-S-nummers worden niet meer gebruikt. **Let op bij externe rapporten:** het verdiepingsrapport van 31-07-2026 verwijst nog naar de oude WP-S-nummers; globale vertaling: WP-S2 (uitnodigingsflows) → R1/R2, WP-S3 (navigatieregister) → R2, WP-S4 (Profiel/Lichaam + ouderhome) → R1 (+ geparkeerd deel ná R8), WP-S7 (trainerdesktop) → R3, WP-S8 (club/hoofdtrainer/clubbeheerder) → R3/R4, WP-S9 (mechanieker) → R5.
...
- **Rechten (stap 1) — VERIFICATIE UITGEVOERD 31-07-2026:** broncode-inspectie bevestigt dat de write-endpoints (`POST /api/races`, training-inserts in `routes/athlete.ts`) alleen onder het **eigen** clerkId schrijven — géén cross-account-lek, dus geen spoed-veiligheidsfix nodig. Er is wél geen server-side rolgate voor `parent`; het dichtzetten (`POST` training/race als parent ⇒ 403) is regulier R1-bouwwerk en wacht op plangoedkeuring.
