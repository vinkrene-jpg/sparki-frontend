...
| Fail-closed gedrag | bestaand, **gedeeltelijk** | server: `lib/entitlements.ts` L81 (`status !== "active"` ⇒ false); onbekende `entitlementMode` wordt naar `subscription` genormaliseerd (L130–132); leesfout op `user_entitlements` zet `degraded=true` (L140–150) | Genuanceerd: niet-actieve persoonlijke entitlements en corrupte tier/status vallen dicht (bewijs: `test:entitlements` 19/19, stripe-billing "corrupte tier/status → FREE"). **Maar** na een `degraded`-leesfout blijven variant-/tier-grants gewoon meetellen (`hasCommercialFeature` negeert `degraded`, L333–339), en een onbekende modus met geldige `productVariant` haalt gewoon variantrechten op. Een storing kan dus nooit rechten *toevoegen*, maar neemt bestaande variantrechten ook niet weg. Frontend faalt bewust open (`use-feature-access.ts`) — veilig omdat de server de poort is. | klein vervolgstapje: vastleggen of `degraded` variant-grants zou moeten blokkeren (strikte lezing van INV-4) |
...
| INV-4 onbekend ⇒ laagste niveau | **GEDEELTELIJK** | Onbekende gebruiker, niet-actieve persoonlijke entitlements en corrupte tier/status vallen dicht (scenario's groen 31-07: "onbekende modus/gebruiker fail-closed", "corrupte tier/status → FREE"). Maar: bij een `degraded`-leesfout op `user_entitlements` blijven variant-/tier-grants meetellen (`hasCommercialFeature` negeert `degraded`, L333–339), en een onbekende `entitlementMode` met geldige `productVariant` levert gewoon variantrechten. Een storing voegt nooit rechten toe (nooit "stilzwijgend Compleet" — het tegenvoorbeeld wordt geweigerd), maar het systeem zakt óók niet altijd naar het laagste niveau. → besluit nodig of de strikte lezing gewenst is (zie Spoor A). |
