...
- **Open technisch risico:** niet geverifieerd of de write-endpoints (training/races/profiel) server-side een aparte rolcheck voor `parent` hebben. Zolang dat niet bewezen is, is "eigen training toevoegen" mogelijk óók een schrijfrechten-lek, niet alleen een UI-fout. Verificatie heeft voorrang (WP-R1, stap 1).
