...
**Volgorde:** start na `SPARKI_BUILD_01` en na F1 t/m F3 van `SPARKI_BUILD_02` — het dagschema en het wedstrijdplan draaien op de werkobjectlaag.
...
**Mirror:** het laatste punt is het zwaarste — elke plek waar een operationele rol een medisch veld kan lezen, is een bevinding. **F1 start pas na `F0 MIRROR_PROVEN`.**
...
**Blokkerende input:** de rolwaarde `nutrition_specialist` bestaat server-side (pakket 01, F3). Zonder die rolwaarde blijven de voedingsdocumenten `OPEN`.
...
- **Prijs en publieke vrijgave van Team.** Het bedrag van €149 is gebouwd zonder vastgelegd prijsbesluit. Dat besluit ligt bij René. Tot dan blijft Team **niet publiek vrijgegeven**, ongeacht het F12-bewijs.
- **Regels voor automatische reservepromotie.** BB-45 verbiedt promotie zonder ingestelde regel. Óf die regels er komen en hoe ze eruitzien, is een besluit van René. Tot dan: geen promotie, alleen tonen.
- **Minimale bezetting per evenementtype.** Het conflicttype "minimale bezetting ontbreekt" heeft een norm nodig. Configureerbaar bouwen; de waarden komen van de organisatie, niet van Replit.
- **Bewaartermijnen** voor evenement-, bezettings- en inzagelogs. Vastgelegd in de bewaarmatrix PD-11 uit pakket 02, centraal configureerbaar, niet hardcoded. De waarden komen van jurist of accountant.
