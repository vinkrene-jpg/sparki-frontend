...
**Volgorde:** dit pakket gaat vóór `SPARKI_BUILD_02`, `03` en `04`. Die drie bouwen erop voort.
...
**Mirror:** steekproef van vijf "aanwezig" tegen code en drie "afwezig" waarbij Mirror zelf zoekt; alle server-side rolwaarden benoemd. **F1 start pas na `F0 MIRROR_PROVEN`.**
...
**Blokkerende input:** de inventarisatie uit F0 van bestaande personen-, klant-, leden- en organisatiegegevens.
...
Niet gebouwd: <wat bewust is overgeslagen>
Migratie: <uitgevoerd / n.v.t.> — telling voor/na
Tests: <groen/rood> build · typecheck · unit · e2e
Bewijs: <verwijzing>
Blokkade: <geen / welke>
```
...
Waar een echte keuze ontbreekt, wordt **niet zelf ingevuld**:

- **Zes bewaartermijnen** (profiel- en accountdata · betalings- en factuuradministratie · auditlogs · communicatie en supporttickets · gezondheids- en hersteldata · back-uprotatie). Ligt bij jurist of accountant. Retentievelden worden **configureerbaar** gebouwd, centraal, nooit verspreid hardcoded. Een betaalde publieke release blijft geblokkeerd zolang deze onbepaald zijn.
- **Stilte-uren:** de tijdvakken zelf zijn een productbesluit van René. De uitzondering voor urgente veiligheidsmeldingen ligt wél vast en wordt gebouwd. Tot het besluit: configureerbaar, waarde leeg.
- **Welke rollen verplicht een VOG nodig hebben.** Vastgelegd is dát het moet waar de jeugdregels dat vereisen; de exacte rollenlijst is een besluit van René. Tot dat besluit wordt de controle gebouwd en per rol **uit** gezet.
