...
**Volgorde:** start na `SPARKI_BUILD_01` en na F1 t/m F6 van `SPARKI_BUILD_02` — trainerdocumenten draaien op de werkobjectlaag en de factuur-PDF op de rapportgenerator.
...
**Mirror:** de eerste vijf vragen bepalen of de zelfstandige trainer als rol überhaupt bestaat. **F1 start pas na `F0 MIRROR_PROVEN`.**
...
Migratie: telling voor/na, niet-omgezet: <aantal en reden>
Tests: <groen/rood>
Bewijs: <verwijzing>
Blokkade: <geen / welke>
```
...
Waar een echte keuze ontbreekt, wordt **niet zelf ingevuld** en niets door Replit verzonnen:

- **Prijs van het trainerabonnement.** Het abonnement mag betaald zijn (BB-60), het bedrag is niet vastgesteld. **Configureerbaar bouwen als productbesluit**, met een lege waarde tot René hem vaststelt. Betaald gebruik gaat pas open na dat besluit.
- **Sporterlimiet per abonnement**, en wat er gebeurt bij overschrijding. Configureerbaar bouwen, waarde leeg.
- **Wettelijke bewaartermijnen**, met name voor facturen en creditnota's. **Vóór implementatie laten bevestigen en centraal configureren; niet verspreid hardcoden.** Een betaalde publieke release blijft geblokkeerd zolang deze onbepaald zijn.
- **Activering van de betaallink** via een verbonden accountmodel: pas na technische én juridische verificatie. Tot dan werkt de module volledig met handmatige betaalstatus.
- **Betalingstermijn als standaard.** Configureerbaar per trainer; er wordt geen standaardwaarde verzonnen.
- **Ondertekening** van de overeenkomst: digitaal, akkoordveld, of buiten Sparki. Tot dat besluit wordt de overeenkomst als document opgeleverd **zonder** ondertekenfunctie.
