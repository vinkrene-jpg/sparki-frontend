...
Ketensluiting daarna: de poort dekt alleen NIEUWE lusgeneratie — opgeslagen routes, handmatige waypoints/A-B en navigatiestart lopen erbuiten. Gedicht: remarks-endpoint levert blockage-samenvatting (zelfde meting, countRouteObstacles pure helper), web toont "Geblokkeerd" + verbergt NAVIGEER, navigatie-start weigert 409, waypoints/PTP weigert 422 (beide cachetakken!). GPX-import en race-koppeling bewust niet geblokkeerd (eigen bestand / hergebruik). Null-meting = overal eerlijk fail-open + log.
