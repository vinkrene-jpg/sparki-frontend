...
**How to apply:** bij elke "mag dit live"-controle: (1) diff sinds laatste publish-commit, (2) 2× verse build + grep dist op dev-sporen én verwachte assets, (3) curl live bundle voor bestaande-configuratievragen, (4) DB-vraag = zitten er schema-bestanden in de diff én raakt de diff de api-server, (5) **flag-gated features: check ook of de flag-rij in de PROD-db bestaat en aan staat voor de doelgroep — build live ≠ feature zichtbaar.** No-row = uit voor iedereen incl. hoofdtester; de /_dev-previewroutes tonen zulke schermen ONGEACHT de flag, dus preview-bewijs zegt niets over flag-status.
