...
- **"Webhook-eerst" zonder geregistreerde webhook = stille rot.** Strava `push_subscriptions` was in prod LEEG (nooit geregistreerd via de API), de lazy catch-up hing alleen aan de koppelingenlijst-route en het app-brede vangnet vereist een nooit-geconfigureerde Scheduled Deployment → gebruikersdata bevroor een maand zonder foutmelding.
  **Why:** een gebruiker die alleen Vandaag/Activiteiten opent raakte nooit een syncpad; het leek "mock data".
  **How to apply:** hang `maybeScheduleStravaCatchUp` fire-and-forget aan de hoofd-leespaden (GET /dashboard + GET /sessions) — zelfde les als news-freshness: self-heal op het LEESPAD, nooit alleen op een aparte pagina of externe cron. Webhook-registratie zelf blijft een operationele stap (API-call met callback-URL) — check `push_subscriptions` bij sync-klachten.
- **Leespad-catch-up heeft een retry-cooldown nodig.** `shouldCatchUp` accepteert `lastRunStartedAt` en weigert (`recent_geprobeerd`) binnen `RETRY_COOLDOWN_MIN` (15 min) na de laatste poging — anders vuurt elke app-opening tijdens een Strava-storing een verse poging en eet het gedeelde rate-limit (100 req/15min) op.
