...
- **Rolmapping definitief (01-08-2026, SPARKI-BESLUIT-2026-010):** `ploegleider` is een APARTE clubrol naast `teammanager` (zelfde trainings- + team-scoped rechten; elke plek die op `teammanager` gate't — permissions én members-GET-scope — moet ploegleider expliciet meenemen). `medic` heet `medical_staff` met beschrijvend `medicalSpecialty` (allowlist, geen rechten, server-side gewist bij rolwissel). Labels: member="Sporter", alleen_lezen="Gast".
- Rollen soigneur/medical_staff zijn additief least-privilege (geen beheer, geen consent-data); web-`ClubRole`-union + `ROLE_LABELS` moeten elke nieuwe rol expliciet meekrijgen of de UI kan hem niet tonen/toewijzen.
