...
**Gewone fiets deelt de 0%-onverhard-grens (taak #441):** `hardRejectIfNeeded` + de +1000-selectiestraf gelden nu voor `cycling-road` ÉN `cycling-regular` — René: 0% onverhard voor racefiets én gewone fiets. Let op: `profile-selection.ts` mapt óók bikeType "gravel" naar `cycling-regular`, dus gravelrijders krijgen dezelfde harde afkeur; gravel heeft geen eigen profiel. Als gravel weer onverhard mág, is een profiel-split of uitzonderingsvlag nodig, niet het terugdraaien van de poort.

## Meerdere voorstellen (30-07-2026)
De kaart-planner (/generate, loop) geeft naast de winnaar max 2 "alternates" terug — dezelfde interne pool-kandidaten die vroeger stil weggegooid werden. Poorten: geen harde straf (score<500), racefiets volledig verhard, pathOverlapFraction<=0.2, pathSharedFraction (cel-overlap tussen twee paden) <=0.6 t.o.v. winnaar én elkaar, en dezelfde hardRejectIfNeeded-poort (afgekeurd = stil overslaan, nooit aanbieden). Elk voorstel krijgt een EIGEN avoidReport (N-weg-aandeel verschilt per lus). Cache-hit = eerlijk geen alternatieven. /generate/options blijft alternates-loos (toont al afstandsvarianten).
