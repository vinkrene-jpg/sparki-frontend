# Sparki — Eerlijk verdienmodel & gezonde retentie

*Strategiedocument. Doel: bewijzen dat "eerlijk" en "winstgevend" hier samengaan, en vastleggen hóé — als leidraad voor de komende sprints.*

---

## 1. De kern: waarvoor je betaald krijgt bepaalt of eerlijkheid kan

De verslavende apps (socials, gokken, oneindige feeds) verdienen aan **aandacht**: hoe langer je scrolt, hoe meer zij verdienen. Bij hen botst eerlijkheid frontaal met de omzet — hun winst zít in jouw verspilde tijd. Daarom kunnen zij niet eerlijk zijn.

Sparki verdient niet aan schermtijd. Sparki verdient aan **resultaat en vertrouwen**:

- Een renner die *meetbaar* sneller wordt, minder blessures krijgt, z'n doel haalt.
- Een ouder die z'n kind veilig en verantwoord ziet trainen.
- Een coach die tijd bespaart en meer renners aankan.

Bij dit model is eerlijkheid geen rem maar de **motor**. Het is precies wat generieke trainingsapps níét leveren, en het is de reden dat iemand blijft betalen en het doorvertelt. De vraag is dus niet "eerlijk óf winstgevend", maar: *bouw je een verdienmodel op aandacht (dan moet je liegen) of op resultaat (dan moet je eerlijk zijn)?* Wij kiezen het tweede.

---

## 2. Gezonde retentie — de wet (wat mag, wat niet)

"Niet verslavend" betekent níét "passief wachten tot de gebruiker komt". Dat is de valkuil. Het verschil zit in de **reden** waarom je iemand terughaalt.

### Toegestaan — retentie via echte waarde
- **Nieuwe, échte inzichten** melden ("Je vorm is deze week meetbaar gestegen — hier is waarom"), en alleen als er echt iets nieuws is.
- **Timing op ontvankelijkheid**: porren op het moment dat de renner zelf gewend is te openen (de engagement-engine die er al is), niet op willekeurige piek-momenten.
- **Voortgang zichtbaar maken**: doelen, ontwikkeling, mijlpalen die daadwerkelijk gehaald zijn.
- **Zinvolle herinneringen**: training vandaag, wedstrijd nadert, herstel nodig.
- **Sociale waarde met consent**: samen trainen, coach-feedback, ouder-inzicht — altijd privacy-gated.

### Verboden — dark patterns (expliciet, blijvend)
- Nep-urgentie of nep-schaarste ("nog 2 plekken!", "3 mensen bekeken je profiel").
- Willekeurige beloningen / variabele "gok"-mechaniek (streaks die straffen, verrassings-badges om terug te lokken).
- FOMO-druk, guilt-tripping ("je bent 5 dagen weggeweest 😢"), of oneindige feeds zonder eind.
- Verstoppen van de opzeg-/uitzet-knop; standaard alles-aan zonder duidelijke opt-out.
- Meldingen die geen echte, nieuwe waarde bevatten.

### De toetssteen (één vraag per feature)
> *"Haalt dit iemand terug voor iets dat écht klopt en hem verder helpt — of voor een truc?"*
Als het antwoord "truc" is, bouwen we het niet. Urgente/gezondheids-meldingen (blessure, overbelasting) staan altijd bovenaan en worden nooit door commerciële of engagement-logica verdrongen.

---

## 3. Het verdienmodel — realistisch en haalbaar

De wielermarkt (jeugd) is een **niche**: relatief klein, prijsgevoelig bij de renner zelf. De sleutel is dat **niet de jeugdrenner de hoofdbetaler is**, maar de mensen om hem heen die waarde én budget hebben: **ouders, coaches en clubs**. De renner krijgt genoeg gratis waarde om te blijven; het geld komt van wie er baat bij heeft dat de renner goed begeleid wordt.

### Doelgroepen & wie betaalt

| Wie | Wat ze willen | Betalingsbereidheid |
|---|---|---|
| Jeugdrenner | Beter worden, plezier, geen gedoe | Laag (blijft op gratis) |
| Ouder | Kind veilig/verantwoord zien trainen, overzicht | **Middel-hoog** |
| Coach | Tijd besparen, meer renners aankunnen, betere begeleiding | **Hoog** (het is z'n werk/passie) |
| Club / team / federatie | Talent ontwikkelen, uniforme aanpak, ledenbinding | **Hoog** (B2B, schaalbaar) |

### Voorgestelde lagen (indicatieve prijzen, NL/EU)

1. **Gratis — "Renner"** (acquisitie-motor)
   Kernwaarde die écht werkt: trainingsschema, activiteiten koppelen (Strava e.d.), basis-inzicht, doelen. Genoeg om te blijven en door te vertellen. Dit is bewust royaal — het is je groeikanaal, geen uitgeklede lokker.

2. **Renner Premium — ~€6/mnd of ~€59/jaar**
   Diepere analyse, voedings-/herstel-begeleiding, wedstrijd-intelligentie, langetermijn-ontwikkelmodel. Voor de gemotiveerde (vaak oudere) renner die zelf investeert.

3. **Ouder-inzicht — ~€4/mnd, of gratis meegeleverd bij Premium**
   Veilig, privacy-gated venster op de training van het kind (afgestemde deelniveaus die er al zijn). Gemoedsrust als product. Vaak de laagdrempeligste eerste betaler.

4. **Coach — ~€4–5 per gekoppelde renner/mnd, met staffel** (bijv. flat ~€29/mnd tot 10 renners, daarna per renner)
   De coach begeleidt méér renners in minder tijd: advies-schema's, roster-overzicht, gedeelde context. Hoogste betalingsbereidheid, want het is direct rendement op zijn tijd.

5. **Club / team / federatie — B2B, ~€250–750/jaar per club** (afhankelijk van omvang)
   Alle renners + coaches van een club onder één afspraak, uniforme aanpak, talentontwikkeling. Dit is de **schaalbare** hefboom: één deal = tientallen actieve gebruikers. Denk clubs, regionale selecties, op termijn een KNWU-achtige partner.

> Prijzen zijn richtpunten om mee te starten en te testen, geen definitieve tarieven.

---

## 4. Waarom dit financieel wérkt

- **Retentie ís het verdienmodel.** Bij abonnementen bepaalt vasthouden (lage churn) vrijwel alles. En eerlijke, op-resultaat-gebaseerde waarde geeft juist lage churn: mensen zeggen niet op wat hen aantoonbaar beter maakt. De dark patterns die we weglaten, verhogen juist churn op termijn (irritatie, wantrouwen).
- **Betaler ≠ eindgebruiker.** Doordat ouders/coaches/clubs betalen en de jeugdrenner gratis blijft, is de niche-prijsgevoeligheid van de renner geen blokkade. De gratis laag is het groeikanaal; de betaalde lagen zitten waar het budget zit.
- **Lage marginale kosten.** Software schaalt: elke extra club/coach kost weinig extra. De grootste variabele kost is de reken-/denkkracht per gebruiker — te beheersen door zware analyses te cachen en alleen te draaien als er nieuwe data is (patroon dat al in de app zit).
- **B2B als versneller.** Één clubdeal levert veel actieve gebruikers tegen lage acquisitiekosten (via de club, niet één-voor-één adverteren). Dat drukt de kosten om een klant te winnen fors.

**De simpele redenering:** als een coach €29/mnd betaalt en gemiddeld 2+ jaar blijft omdat het hem echt tijd/resultaat oplevert, verdien je die klant ruim terug — zónder één truc. Dat is het bewijs dat eerlijk niet "waardeloos" hoeft te zijn.

---

## 5. Eerlijk gezegd — de risico's (geen mooipraterij)

- **Nichegrootte.** Jeugdwielrennen NL is klein. Groei zit in (a) breder trekken naar andere disciplines/sporten en (b) internationaal via clubs — beide staan al op de roadmap (multi-sport).
- **Gratis laag te royaal of te karig.** Te karig = geen groei; te royaal = niemand upgradet. Dit moet je meten en bijstellen, niet gokken.
- **B2B heeft een lange verkoopcyclus.** Clubs/federaties beslissen traag. Start daarom parallel met directe ouder/coach-abonnementen voor snellere inkomsten.
- **Vertrouwen is fragiel.** Eén verborgen dark pattern of privacy-misstap ondermijnt precies datgene waar je model op rust. De "wet" in sectie 2 is dus geen ethisch sausje maar bedrijfsbescherming.

---

## 6. Concrete eerste stappen

1. **Leg de retentie-wet (sectie 2) vast** als product-principe in `replit.md`, zodat elke sprint eraan getoetst wordt.
2. **Bouw de betaal-laag pas ná bewezen waarde**: eerst zichtbaar maken dat de app renners meetbaar verder helpt (voortgang, resultaten), dán de abonnementen aanzetten. Betalen voor bewezen waarde is eerlijk; betalen voor een belofte niet.
3. **Begin met de laagdrempeligste betaler**: ouder-inzicht en coach, want daar zit bereidheid én snel rendement. Club/B2B parallel opstarten (langere adem).
4. **Meet de juiste dingen**: retentie/churn en of gebruikers écht beter worden — niet schermtijd. De metriek volgt het principe.

---

### Samengevat
We kiezen bewust een derde spoor: **hoge retentie via echte waarde + een verdienmodel dat op resultaat rust, niet op aandacht.** Eerlijkheid is hier geen offer maar het verschil dat je verkoopt. Het is haalbaar omdat de betalers (ouders, coaches, clubs) waarde én budget hebben, en omdat vertrouwen precies de lage churn oplevert waar een abonnementsmodel op draait.
