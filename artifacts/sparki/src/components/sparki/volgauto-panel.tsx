...
export function VolgautoPanel({
  ...
  return (
    <div
      className={`rounded-xl border border-white/[0.07] bg-[#070d16]/[0.82] p-4 backdrop-blur-md ${className ?? ""}`}
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          className={`relative h-6 w-11 shrink-0 rounded-full transition ${
            enabled ? "bg-cyan-400/80" : "bg-white/15"
      ...
      {enabled && plan && (
        <div className="mt-4 space-y-3 text-sm text-white/80">
          {plan.outdated && (
            <p className="rounded-lg bg-amber-400/10 p-2 text-xs text-amber-200/90">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div>
              <div className="text-[11px] uppercase tracking-wide text-white/45">Fietsroute</div>
              <div className="font-mono">{bikeDistanceKm != null ? fmtKm(bikeDistanceKm) : "—"}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wide text-white/45">Autoroute</div>
              <div className="font-mono">{plan.carDistanceKm != null ? fmtKm(plan.carDistanceKm) : "—"}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wide text-white/45">Gedeeld</div>
              <div className="font-mono">{sharedKm != null ? fmtKm(sharedKm) : "—"}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wide text-white/45">Splitsingen</div>
              <div className="font-mono">{splitCount}</div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="text-[11px] uppercase tracking-wide text-white/45">Fiets (geschat)</div>
              <div className="font-mono">{fmtMin(bikeEtaSec)}</div>
            </div>
            <div>
              <div className="text-[11px] uppercase tracking-wide text-white/45">Auto (geschat)</div>
            ...
            <div>
              ...
              <ul className="mt-1 space-y-1">
                {plan.meetpoints.slice(0, 8).map((m, i) => (
                  <li key={`${m.bikeKm}-${i}`} className="flex items-baseline gap-2 text-xs text-white/70">
                    <span className="font-mono text-white/45">km {m.bikeKm.toFixed(1).replace(".", ",")}</span>
                    <span>{m.name}</span>
                    {m.source === "parkeerplaats" && (
                      <span className="rounded bg-white/10 px-1 text-[10px] text-white/55">P</span>
                    )}
                  </li>
                ))}
          ...
          {plan.dataNotes.map((n) => (
            <p key={n} className="text-xs text-white/50">
              {n}
            </p>
          ))}
          <p className="text-xs text-amber-200/80">{plan.disclaimer}</p>
        </div>
      )}

      {enabled && planQuery.isLoading && (
        <p className="mt-3 text-xs text-white/55">Volgautoplan laden…</p>
      )}
    </div>
  )
}
