  ...
  return (
    <div className="flex items-start gap-3 py-3.5">
      ...
      <div className="flex-1">
        <div className="text-[14px] tracking-tight text-foreground/85">{title}</div>
...
function Select<T extends string>({
  ...
  return (
    <select
      ...
      className="rounded-lg border border-border bg-card px-2.5 py-1.5 font-sans text-[12px] text-foreground/75 outline-none disabled:opacity-40"
    >
      {options.map((o) => (
...
export function PrivacySettingsSection() {
  ...
  return (
    <section className="pt-2">
      ...
      <Link
        href="/ai-toestemming"
        className="mt-3 flex items-center gap-3 rounded-2xl border border-border bg-card px-4 py-3 backdrop-blur-md transition-colors hover:border-accent-cyan/30"
      >
        <span
          className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border"
          style={{
        ...
        >
          <ShieldCheck className="h-4 w-4 text-foreground/55" strokeWidth={1.75} />
        </span>
        <span className="flex-1">
          <span className="block text-[14px] tracking-tight text-foreground/85">
