...
function ClimbRow({
  ...
  return (
    <button
      ...
      className="flex w-full items-center gap-3 rounded-2xl border border-border bg-card p-4 text-left backdrop-blur-md transition-colors hover:border-border"
    >
      <div
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate font-sans text-[14px] text-foreground/90">
          {climb.name}
        </p>
  ...
  return (
    <div>
      <button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground transition hover:text-foreground/80"
      >
        <ArrowLeft className="h-3.5 w-3.5" /> Terug naar zoeken
      </button>

      <h2 className="mt-4 font-sans text-[20px] text-foreground/95">{climb.name}</h2>
                ...
                <p className="mt-3 flex items-center gap-1.5 text-[11px] text-muted-foreground">
                  ...
                  {data.description.sourceUrl && (
                    <a
                      ...
                      className="inline-flex items-center gap-1 text-muted-foreground underline decoration-white/20 underline-offset-2 hover:text-foreground/80"
                    >
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-border bg-muted px-3 py-2">
      ...
      <p className="mt-0.5 font-sans text-[15px] text-foreground/90">{value}</p>
    </div>
  )
}
...
function Explorer() {
  ...
  return (
    <div>
      ...
      >
        <div className="flex items-center gap-2 rounded-2xl border border-border bg-card px-3.5 py-3 backdrop-blur-md">
          ...
          <input
            ...
            className="min-w-0 flex-1 bg-transparent font-sans text-[14px] text-foreground/90 placeholder:text-muted-foreground focus:outline-none"
          />
        </div>
        <div className="flex items-center gap-2 rounded-2xl border border-border bg-card px-3.5 py-3 backdrop-blur-md">
          ...
          <button
            ...
            className={`rounded-full border px-3 py-1 font-mono text-[11px] transition ${
              radiusKm === r
                ...
                : "border-border text-muted-foreground hover:border-border hover:text-foreground/75"
            }`}
          >
            {r} km
          </button>
        ...
        {debouncedArea.trim().length < 2 ? (
          ...
          <div className="rounded-2xl border border-border bg-card p-4 backdrop-blur-md">
            ...
            <button
              type="button"
              onClick={() => refetch()}
              className="mt-3 rounded-xl border border-border px-4 py-2 font-sans text-[12px] text-muted-foreground transition hover:border-border"
            >
              Opnieuw proberen
            </button>
          </div>
