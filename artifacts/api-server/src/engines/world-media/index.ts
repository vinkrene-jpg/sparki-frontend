...
//
// All Sparki World media is system-owned and registered through the CENTRAL
// bestandslaag (lib/files.ts) as a PUBLIC file: any signed-in user can read it
// through the central serve-pad (/api/storage → serveFile, met rechtencheck vóór
// streamen), which is correct for a shared feed and keeps it walled off from
// private, user-owned objects. There is no module-eigen opslag-/serve-pad meer;
// intrekking (files.revokedAt) en retentie lopen centraal. De cache-first
// semantiek (promptKey UNIQUE, ready-rijen nooit opnieuw genereren) blijft
// volledig intact — dit is verhuizen, niet herbouwen.
//
...
import {
  registerWorldMedia,
  registerWorldMediaFromObjectPath,
  deleteFileRow,
} from "../../lib/files";
...
//
// F11-01: `upload` registreert de bytes via de CENTRALE bestandslaag en geeft
// zowel het (centrale) objectPath als de gekoppelde fileId terug. Tests mogen
// een stub geven die alleen een objectPath teruggeeft (fileId dan null).
export type MediaUploadResult = { objectPath: string; fileId?: number | null };
export type MediaDeps = {
  generate?: (prompt: string) => Promise<{ b64_json: string; mimeType: string }>;
  upload?: (bytes: {
    base64: string;
    mimeType: string;
  }) => Promise<string | MediaUploadResult>;
};
...
// ── upload (via de centrale bestandslaag, F11-01) ─────────────────────────────
// Registreer de gegenereerde bytes via de CENTRALE laag (lib/files.ts) als
// systeem-eigen, PUBLIEK bestand. Er is geen module-eigen opslagpad meer: beeld
// gaat door de scan-/her-encode-poort, video wordt centraal geregistreerd zonder
// her-encode. Retourneert het centrale objectPath én de gekoppelde fileId zodat
// de virtual_media-rij naar de files-rij (bron van waarheid) kan wijzen.
async function uploadPublic(bytes: {
  base64: string;
  mimeType: string;
}): Promise<MediaUploadResult> {
  const reg = await registerWorldMedia({
    base64: bytes.base64,
    mimeType: bytes.mimeType,
    originalName: "world-media",
  });
  if (!reg.ok) {
    throw new Error(`Opslaan van world-media mislukt (${reg.status}): ${reg.reason}`);
  }
  return { objectPath: reg.file.objectPath, fileId: reg.file.id };
}

// ── centrale koppeling (F11-01 lazy backfill) ─────────────────────────────────
// Zorgt dat een READY virtual_media-rij een centrale files-koppeling (fileId)
// heeft. Rijen van na de omlegging hebben die al; bestaande rijen krijgen hem
// hier lui bij de eerstvolgende serve. De bytes staan al publiek in storage —
// we registreren ze alsnog centraal en wijzen objectPath naar het (centrale)
// files-object.
//
// RACE-VEILIG & IDEMPOTENT: twee gelijktijdige serves op dezelfde rij mogen NIET
// elk een files-rij achterlaten. Daarom claimen we de virtual_media-rij ATOMAIR
// ná registratie via een conditionele UPDATE (... WHERE id = ? AND file_id IS
// NULL). Wint deze aanroep de claim niet (een concurrent was eerder), dan ruimen
// we onze zojuist geregistreerde files-rij direct op → exact één files-rij per
// virtual_media-rij. Retourneert de gekoppelde rij, of null als koppelen (nog)
// niet lukte (de aanroeper faalt dan fail-closed i.p.v. de rauwe URL te tonen).
async function ensureCentralLink(row: VirtualMedia): Promise<VirtualMedia | null> {
  // Al gekoppeld: idempotent klaar.
  if (row.fileId != null) return row;
  if (!row.objectPath) return null;
  let reg;
  try {
    reg = await registerWorldMediaFromObjectPath(row.objectPath);
  } catch {
    return null;
  }
  if (!reg.ok) return null;

  // Atomaire claim: alleen de EERSTE concurrent zet file_id; de rest raakt 0 rijen.
  let updated: VirtualMedia | undefined;
  try {
    [updated] = await db
      .update(virtualMediaTable)
      .set({ fileId: reg.file.id, objectPath: reg.file.objectPath })
      .where(
        and(eq(virtualMediaTable.id, row.id), isNull(virtualMediaTable.fileId)),
      )
      .returning();
  } catch {
    updated = undefined;
  ...
  // Claim verloren (of rij verdween): onze files-rij is een wees → opruimen zodat
  // er geen tweede files-rij blijft bestaan. Lees daarna de winnende rij terug.
  try {
    await deleteFileRow(reg.file.id);
  } catch {
    // Opruimen faalde: niet fataal (de winnende koppeling is leidend), niet tonen.
  }
  const [winner] = await db
    .select()
    .from(virtualMediaTable)
    .where(eq(virtualMediaTable.id, row.id))
    .limit(1);
  return winner?.fileId != null ? winner : null;
}
...
export async function resolveMedia(
  ...
  if (existing && existing.status === "ready" && existing.objectPath) {
    ...
    // F11-01 lazy backfill: een READY rij van vóór de omlegging heeft nog geen
    // centrale files-koppeling. Koppel hem alsnog bij deze (eerstvolgende) serve,
    // zodat óók bestaande media centraal beheerd/intrekbaar wordt.
    // FAIL-CLOSED: lukt het koppelen (nog) niet, dan geven we de RAUWE object-URL
    // NIET terug — anders zou het oude pad bereikbaar blijven zonder centrale
    // retentie/intrekbaarheid (strijdig met F11). We maskeren dan objectPath naar
    // null (media telt als niet-beschikbaar); een volgende resolve mag hem, dankzij
    // cache-first, alsnog koppelen. Geen stille 500: we slaan eerlijk over.
    const linked = await ensureCentralLink(existing);
    if (!linked) {
      return { ...existing, objectPath: null, reuseCount: existing.reuseCount + 1 };
    }
    return { ...linked, reuseCount: existing.reuseCount + 1 };
  }
  ...
  try {
    ...
    // Normaliseer het upload-resultaat: de centrale laag geeft { objectPath,
    // fileId } terug; een test-stub mag een kaal objectPath teruggeven.
    const objectPath =
      typeof uploaded === "string" ? uploaded : uploaded.objectPath;
    const fileId =
      typeof uploaded === "string" ? null : uploaded.fileId ?? null;
    const [row] = await db
...
export async function readyHighlightUrls(
  ...
  for (const r of rows) {
    ...
    // F11-01 lazy backfill: koppel bestaande clips alsnog centraal bij de serve.
    // FAIL-CLOSED: lukt het koppelen niet, dan publiceren we GEEN URL (de clip telt
    // als afwezig i.p.v. via het oude, niet-intrekbare pad geëxposeerd te worden).
    const linked = await ensureCentralLink(r);
    if (!linked) continue;
    const slug = keyToSlug.get(linked.promptKey);
    const url = mediaUrl(linked.objectPath);
